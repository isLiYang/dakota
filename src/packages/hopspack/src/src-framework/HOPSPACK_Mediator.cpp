// $Id: HOPSPACK_Mediator.cpp 217 2013-11-25 21:59:49Z tplante $ 
// $URL: https://software.sandia.gov/svn/hopspack/tags/dakota-6.2/src/src-framework/HOPSPACK_Mediator.cpp $ 
    ...
    _nNumInitialFailedEvals (0)
{
    ...
    _nMaxInitialFailedEvals
        = cMediatorParams.getParameter ("Max Initial Eval Failures", 5);
    if (_nMaxInitialFailedEvals <= 1)
        _nMaxInitialFailedEvals = 2;
...
void  Mediator::printDebugInfo (void) const
{
    ...
    if (_nMaxEvaluations == -1)
        cout << "  (no limit)";
    cout << endl;
    cout << "  Max Initial Eval Failures = " << _nMaxInitialFailedEvals
         << endl;
...
int  Mediator::citizenExchange_ (const ConveyorList &  cR)
{
    ...
    for (it = _cCitizenList.begin(); it != _cCitizenList.end(); it++)
    {
        ...
        if ((nState == Citizen::CONTINUE) || (nState == Citizen::WAITING))
        {
            ...
            nNumCtznPoints += (int) cNewPoints.size();
...
{
    //---- STOP IF A LARGE NUMBER OF INITIAL EVALUATIONS HAVE FAILED.
    if (_nNumInitialFailedEvals != -1)
    {
        list< DataPoint * >::const_iterator  it;
        for (it = cList.begin(); it != cList.end(); it++)
        {
            //---- DO NOT CONSIDER UNEVALUATED OR CACHED POINTS.
            if ((*it)->getState() == DataPoint::EVALUATED_FC)
            {
                if ((*it)->getBestF() != dne())
                {
                    //---- ANY SUCCESSFUL EVALUATION CANCELS THE STOP TEST.
                    _nNumInitialFailedEvals = -1;
                    break;
                }
                else
                {
                    _nNumInitialFailedEvals++;
                }
            }
        }
        if (_nNumInitialFailedEvals >= _nMaxInitialFailedEvals)
        {
            if (Print::doPrint (Print::FINAL_SOLUTION))
            {
                cout << endl;
                cout << "Mediator stopping - initial " << _nMaxInitialFailedEvals
                     << " evaluations failed" << endl;
                cout << endl;
            }
            return( true );
        }
